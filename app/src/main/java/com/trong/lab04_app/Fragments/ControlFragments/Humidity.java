                ...
                call.enqueue(new Callback<DHTmodel>() {
                    ...
                    public void onResponse(@NonNull Call<DHTmodel> call, @NonNull Response<DHTmodel> response) {
                        if (response.isSuccessful()) {
                            DHTmodel humidityResponse = response.body();
                            if (humidityResponse != null) {
                                double humidity = humidityResponse.getHumidity();
                                humidityTextView.setText(Double.toString(humidity));
                            }
                        } else {
                            // Handle null response body
                            Toast.makeText(getContext(), "Humidity data is empty", Toast.LENGTH_SHORT).show();
                        }
